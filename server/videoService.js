...
class VideoService {
  ...
  static async createVariedPrompts(postContent, platform, brandData, userHistory) {
    ...
    // Create professional brand-aligned prompts
    const prompts = await this.createProfessionalPrompts(postContent, platform, brandData, selectedThemes, selectedStyles);
    
    return [
      {
        type: `${selectedThemes[0]} - Cinematic Business Trailer`,
        content: prompts.primary,
        duration: '10s',
        style: 'Professional cinematic movie trailer with dramatic lighting and high visual fidelity',
        theme: selectedThemes[0],
        visualStyle: selectedStyles[0],
        autoGenerated: false
      },
      {
        type: `${selectedThemes[1]} - Strategic Vision`,
        content: prompts.secondary,
        duration: '10s',
        style: 'Strategic business transformation with vibrant artistic scenes',
        theme: selectedThemes[1],
        visualStyle: selectedStyles[1],
        autoGenerated: false
      },
      {
        type: `Custom Brand Journey - ${selectedThemes[0]}`,
        content: prompts.brandJourney,
        duration: '10s',
        style: 'Hero\'s journey visualization with dynamic business elements',
        theme: selectedThemes[0],
        visualStyle: selectedStyles[0],
        autoGenerated: true
      }
  ...
  static async createProfessionalPrompts(postContent, platform, brandData, selectedThemes = [], selectedStyles = []) {
    ...
    return {
      primary: `Generate 10-second cinematic movie trailer for adult business audience, interpreting Strategyzer brand purpose: ${brandPurpose}. Clever art director twist: Visualize strategic intent as hero's journey through vibrant, artistic scenes—${visualElement1}, dynamic transitions. Present tense, quick cuts, low-angle shots, vivid colors, dramatic lighting, high visual fidelity; no animals or child themes. Strategic theme: ${theme1}. Professional business focus showcasing "${postContent.substring(0, 40)}..." Queensland SME market alignment.`,
        
      secondary: `Generate 10-second cinematic movie trailer for adult business audience, interpreting Strategyzer brand purpose: ${brandPurpose}. Clever art director twist: Visualize strategic intent as hero's journey through vibrant, artistic scenes—${visualElement2}, floating business elements. Present tense, quick cuts, low-angle shots, vivid colors, dramatic lighting, high visual fidelity; no animals or child themes. Strategic theme: ${theme2}. Content focus: "${postContent.substring(0, 40)}..." Professional transformation narrative.`,
      
      brandJourney: `Generate 10-second cinematic movie trailer for adult business audience, interpreting Strategyzer brand purpose: ${brandPurpose}. Clever art director twist: Visualize strategic intent as hero's journey through vibrant, artistic scenes—professional executives in modern glass conference rooms, holographic data visualization. Present tense, quick cuts, low-angle shots, vivid colors, dramatic lighting, high visual fidelity; no animals or child themes. Epic business transformation showing "${postContent.substring(0, 40)}..." with Queensland SME success narrative.`
    };
  }
